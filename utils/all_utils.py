...
def prepare_data(df):
    ...
    """
    logging.info("Prepring the data by segregating the independent variables and dependent variables")
    x = df.drop("y", axis=1)
    y = df["y"]
    return x, y
...
def save_plot(df, file_name, model):
    """
    :param df: it is a data_frame
    :param file_name: it is path to save the plot
